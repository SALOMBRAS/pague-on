(() => {
  ...
  function renderLock(message = '') {
    ...
    overlay().querySelector('[data-lock-forgot]')?.addEventListener('click', () => renderLock('Para redefinir o PIN, entre no app e vá em Perfil › Segurança › PIN. O bloqueio é local deste dispositivo.'));
    overlay().querySelector('[data-lock-logout]')?.addEventListener('click', () => { overlay().classList.remove('show'); window.pagueOnAuth?.logout?.(); });
  }
