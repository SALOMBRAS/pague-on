...
async function logout(req, res) { await authService.logout((req.actor || req.user).id); res.clearCookie(refreshCookie, cookieOptions(false)); return sendSuccess(res, null, 'Sessão encerrada com sucesso.'); }
...
async function changePassword(req, res) { const input = passwordSchema.parse(req.body); await authService.changePassword(req.actor || req.user, input.oldPassword, input.newPassword); return sendSuccess(res, null, 'Senha atualizada com sucesso.'); }
