...
// Auditoria fora do caminho crítico: dispara sem bloquear a resposta (fire-and-forget).
// `audit.record` não lança para eventos não-críticos, então este helper é seguro.
function queueAudit(args) { try { audit.record(args).catch(() => {}); } catch (_error) { /* auditoria nunca derruba o login */ } }
...
async function login(req, res) { const { remember, email, ...payload } = req.body || {}; const input = loginSchema.parse({ ...payload, identity: payload.identity || email }); try { const result = await authService.login(input); queueAudit({ eventType: 'login', req, actor: result.user, payload: { persistentSession: remember !== false } }); return sendSession(res, result, 'Login realizado com sucesso.', 200, remember !== false); } catch (error) { queueAudit({ eventType: 'login_failed', req, payload: { identityHash: audit.hash(input.identity) } }); throw error; } }
