...
function installmentDate(firstDueDate, frequency, index, skipSundays, holidays) {
  ...
  while ((skipSundays && dueDate.getUTCDay() === 0) || holidays.has(dateKey(dueDate))) dueDate = addDays(dueDate, 1);
...
function calculateSchedule(input, configuration) {
  ...
  const dueAt = (index) => installmentDate(input.firstDueDate, input.frequency, index, configuration.skipSundays, holidays);
  let schedule;
...
async function simulation(userId, input, options = {}) {
  ...
  const [limit, configuration] = await Promise.all([
  ...
  if (!configuration) throw new HttpError(409, 'LOAN_CONFIGURATION_REQUIRED', 'Selecione uma modalidade ativa e revisada juridicamente.');
  ...
  return { customer: { id: limit.customer.id, name: limit.customer.name, approvedInterestRate: approvedRate, creditLimit: Number(limit.customer.creditLimit || 0), availableLimit: limit.available }, configuration: { id: configuration.id, displayName: configuration.displayName, formulaVersion: configuration.formulaVersion, formulaPolicy: configuration.formulaPolicy, legalReviewReference: configuration.legalReviewReference }, input: effectiveInput, ...calculation, contractPreview: buildContractPreview(limit.customer, configuration, effectiveInput, calculation) };
}
...
async function confirm(userId, actor, input, req = null) {
  return prisma.$transaction(async (tx) => {
    ...
    const debt = await tx.debt.create({ data: {
      ...
      totalAmount: preview.totalCost, principalAmount: preview.totalPrincipal, appliedInterestRate: Number(preview.input.interestRate), loanModality: input.modality, loanTerms: { configurationId: preview.configuration.id, formulaVersion: preview.configuration.formulaVersion, rateOverrideReason: input.rateOverrideReason || null, notes: input.notes || null, operationId }, installmentAmount: preview.schedule[0].total, totalInstallments: input.totalInstallments, frequency: input.frequency, startDate: input.releaseDate, dueDate: preview.schedule[0].dueDate,
    ...
    const contract = await tx.loanContract.create({ data: { userId, debtId: debt.id, customerId: input.customerId, configurationId: preview.configuration.id, contractNumber, modality: input.modality, termsSnapshot: { input: preview.input, totalPrincipal: preview.totalPrincipal, totalInterest: preview.totalInterest, totalCost: preview.totalCost, schedule: preview.schedule, legalReviewReference: preview.configuration.legalReviewReference, cashAllocations: input.cashAllocations }, documentHtml: preview.contractPreview, consentedAt: new Date(), renewalOfDebtId: renewal?.id || null } });
